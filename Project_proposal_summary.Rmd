...
df$mi_driven = round(df$km_driven * 0.6213712)
...
```{r}
# glimpse(df)
#note since our variable of interest is bianary, this does not offer any insight.

linear_model = lm(sold ~ ., data = df)
# linear_model
...
df <- subset(df, select = -sales_id)

df.numeric = df %>% 
  select(c(mileage,selling_price,engine,max_power,mi_driven,sold))



plot = fviz_nbclust(df.numeric, kmeans, method = "wss") #elbow plot
plot

km <- kmeans(df.numeric, centers = 6, nstart = 25)
km

table(km$cluster)

#plot
fviz_cluster(km, data = df.numeric)
...
#had to remove name for some reason
...
#if the prob is 0.42 or higher we will classify as sell
